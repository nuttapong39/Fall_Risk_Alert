...
/* ปกติรูปแบบวันที่ที่ส่งมากำกวม → normalize เป็น Y-m-d หรือใช้ fallback */
...
/* ดึง message id จาก response รูปแบบต่าง ๆ ของ MOPH */
...
/* ===== Flex payload ===== */
if (!defined('PHARM_TITLE'))      define('PHARM_TITLE', 'Lab Alert ห้องยา');
...
// === Report Button Config (กรอกของจริง) ===
...
if (!defined('PHARM_REPORT_SIGN_KEY')) define('PHARM_REPORT_SIGN_KEY', 'put_a_random_secret_here');
  // ใช้ IP/พอร์ตภายในของคุณ


  // สร้าง URL พร้อมลายเซ็นป้องกันแก้ไขพารามิเตอร์
  function build_report_signed_url(array $r): string {
    // ใช้ id จากคิวเป็นหลัก; ถ้าไม่มี ให้พยายามใช้ lab_order_number + hn เป็น fallback
...
function pharmRisk(string $labName, $rawResult): array {
  ...
  $ret = ['level'=>'ok','chip'=>'ปกติ','color'=>'#10B981'];
  if (strpos($lab,'inr') !== false) {
    if ($x === null) return ['level'=>'mid','chip'=>'INR (ตรวจสอบ)', 'color'=>'#F59E0B'];
    if ($x >= 5.0)   return ['level'=>'high','chip'=>'INR สูงมาก','color'=>'#EF4444'];
    if ($x >= 3.5)   return ['level'=>'mid','chip'=>'INR สูง','color'=>'#F59E0B'];
    return $ret;
  }
  if (strpos($lab,'depakin') !== false) {
    if ($x === null) return ['level'=>'mid','chip'=>'Depakin (ตรวจสอบ)', 'color'=>'#F59E0B'];
    if ($x > 150)    return ['level'=>'high','chip'=>'Depakin สูง','color'=>'#EF4444'];
    return $ret;
  }
  if (strpos($lab,'lithium') !== false) {
    if ($x === null) return ['level'=>'mid','chip'=>'Lithium (ตรวจสอบ)', 'color'=>'#F59E0B'];
    if ($x > 1.2)    return ['level'=>'high','chip'=>'Lithium สูง','color'=>'#EF4444'];
    return $ret;
  }
  if (strpos($lab,'phenytoin') !== false || strpos($lab,'dilantin') !== false) {
    if ($x === null) return ['level'=>'mid','chip'=>'Phenytoin (ตรวจสอบ)', 'color'=>'#F59E0B'];
    if ($x > 20)     return ['level'=>'high','chip'=>'Phenytoin สูง','color'=>'#EF4444'];
    return $ret;
  }
  return ['level'=>'mid','chip'=>'ตรวจสอบผล','color'=>'#F59E0B'];
}
...
function buildPharmPayload(array $r): array {
  ...
  $lines = [
    iconRow("🏥","HN", $r['hn'] ?? '-'),
    iconRow("👤","ชื่อ-สกุล", ($r['fullname'] ?? '-') . (isset($r['age']) ? " ({$r['age']} ปี)" : "")),
    iconRow("🧪","Lab", $r['lab_name'] ?? '-'),
    iconRow("📊","ผลตรวจ", $r['result'] ?? '-'),
    iconRow("🗓️","วันที่ออกผล", $r['lab_date'] ?? '-'),
    iconRow("⏰","เวลาออกผล", $r['lab_time'] ?? '-'),
    iconRow("🩺","แพทย์", $r['doctor'] ?? '-'),
    iconRow("🏷️","ประเภทผู้ป่วย", $r['patient_type'] ?? '-')
  ];

  // รายงานครบจริงหรือยัง
  $reportedOk =
  ...
  if ($reportedOk) {
    ...
    $extraBlock[] = [
      ...
      "contents"=>[ riskChip("เภสัชผู้รับผิดชอบ    ", "#2563EB") ]
    ];
    $extraBlock[] = iconRow("🧑","ผู้รายงาน", (string)$r['reported_by_name']);
    $extraBlock[] = iconRow("📅","วันที่รายงาน", th_date($r['reported_date']));
    $extraBlock[] = iconRow("⏱️","เวลารายงาน", substr((string)$r['reported_time'],0,5));
  } else {
    // ยังไม่ได้รายงาน → ปุ่มไปหน้า report
  ...
  // รวบรวม contents ให้เสร็จก่อน (ลดปัญหาวงเล็บ)
  $bodyContents = array_merge(
      ...
      [
        ...
        "contents"=>[
          ...
          [
            ...
            "contents"=>[
              ["type"=>"text","text"=>"Lab Alert ห้องยา","weight"=>"bold","size"=>"lg","color"=>"#111827"],
              $headerBadge
            ]
          ]
        ]
    ...
    [
      ...
      [
        ...
        "contents"=>[[
          "type"=>"text","text"=>"กรุณาทบทวนการปรับยา/เฝ้าระวังอาการไม่พึงประสงค์",
